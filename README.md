...
```
- 监控与缓存：`qa_cli` 默认注入 `QAMonitor`。命中缓存时输出 `[cache hit/<level>]` 前缀；日志与精确缓存写入 `logs/qa_sessions/`（按日滚动 JSONL），语义缓存已下线，缓存命中只依赖精确匹配（TTL 默认 7 天，可通过 `qa.cache.vectordb_version` 统一失效）。
...
我们在内部问答集上对 `top_k / top_n / tau` 以及相似度阈值（默认 0.5）做过一轮微调，实验结果存放在 `tests/tmp_embedding_test_round1.json`，并将较优参数固化到 `ChromaStore.query_with_guardrails`，让默认体验开箱可用。
...
- 结果记录：Embedding 与插入耗时、成功/失败数写入 manifest，便于回溯；测试回归数据保存在 `tests/tmp_embedding_test_round1.json` 以对比调参效果。
