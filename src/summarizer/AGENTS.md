...
- `page_summarizer.py`: builds Chinese JSON prompt per slide; supports text+image (PPTX) or image-only (PDF, `slide_text=None`), parses model output with fallbacks, returns `PageSummary`.
