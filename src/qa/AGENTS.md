...
- `qa_engine.py`: `QAEngine.answer(question, project_name=None, top_k=8, top_n=5, tau=0.5, monitor=None)`；先通过 `QAMonitor` 精确缓存命中，未命中则检索 + LLM 生成。返回结构包含 `answer/sources/status`，并附 `cache_status`/`cache_level`。
